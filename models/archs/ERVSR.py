...
class Network(nn.Module):
    def __init__(self, config, spynet_path=None):
       ...
       # Bidirectionaal Propagation
        ...
        self.lr_resblocks = ConvResBlock(3, config.num_feat, config.num_blocks)
        self.ref_down_resblocks = ConvResBlock_ref(3, config.num_feat, config.num_blocks)
        self.ref_resblocks = ConvResBlock(3, config.num_feat, config.num_blocks)
    ...
    def forward(self, lrs, refs):
        ...
        ref_fea_down = self.ref_down_resblocks(refs)
        ref_fea = self.ref_resblocks(refs)

        lrs_feas[lrs.size(1)//2] = self.encoder_attention([lrs_feas[lrs.size(1)//2], ref_fea_down])[0]
